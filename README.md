...
- **Need for Custom Transformers**: To incorporate feature engineering into a pipeline, these functions must be converted into custom transformers. This ensures that transformations are executed in a consistent, automated manner during both training and prediction phases.
- **Ensuring Compatibility**: Custom transformers allow for smooth integration within the pipeline, preserving the flexibility and reliability of the preprocessing workflow. 
